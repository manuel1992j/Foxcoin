...
uint256 hashGenesisBlock("0x7014d03e4c72770dfb9faf25cb72a60f7747ec1c3d7b4bf808d4e2371da6f3cd");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Foxcoin: starting difficulty is 1 / 2^12
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 900000); // Foxcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2 * 24 * 60 * 60; // Foxcoin: 2 days
static const int64 nTargetSpacing = 2 * 30; // Foxcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Foxcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Foxcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xf87bc07031ddc9b2031bc3b7fa3e6dd31028a5f38ca60cfcd3345bb30934c3d2");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "fox zorro buscando bajas tasas de transferencias";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("042e385aa43d60f960866a7003ee1f08ac35c862c84263bf9314bf8a0ecc5ca491ef73dd920996a90228b7cb0910e63ed3942ef28e455496c2e84748003f22c0f4") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1641022259;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2088680304;

        if (fTestNet)
        {
            block.nTime    = 1641022216;
            block.nNonce   = 385666569;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x706e4adaf7a2e6d2ff2f783b8d0d5733c16a001134b93fe67d5b6532cf62ca43"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc2, 0xb4, 0xdf }; // Foxcoin: increase each by adding 2 to bitcoin's value.
...
void static FoxcoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running FoxcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FoxcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FoxcoinMiner, pwallet));
}
